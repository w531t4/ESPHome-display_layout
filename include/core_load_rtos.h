...
class CoreLoadSensorsRTOS : public esphome::PollingComponent {
 public:
  ...
  void update() override {
    ...
    UBaseType_t n = uxTaskGetSystemState(tasks.data(), tasks.size(), &total_dummy);
    tasks.resize(n);
    ...
    for (const auto &t : tasks) {
      ...
      if (c < 0 || c >= (int)portNUM_PROCESSORS) continue;
 ...
 private:
  struct Sample { uint64_t total=0, idle=0; };
  Sample prev_[portNUM_PROCESSORS] = {};

  void publish_from_delta(int core, uint64_t total_now, uint64_t idle_now, esphome::Sensor *out) {
    uint64_t d_total = (total_now >= prev_[core].total) ? (total_now - prev_[core].total) : 0;
    uint64_t d_idle  = (idle_now  >= prev_[core].idle ) ? (idle_now  - prev_[core].idle ) : 0;
    prev_[core].total = total_now;
    prev_[core].idle  = idle_now;

    if (d_total == 0) return;  // first sample or no change
    float util = 100.0f * (1.0f - (float)d_idle / (float)d_total);
    if (util < 0.f) util = 0.f;
    if (util > 100.f) util = 100.f;
