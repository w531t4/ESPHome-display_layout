...
namespace ui {
    ...
    inline void printf_dual(esphome::display::Display *it,
                            esphome::font::Font *font,
                            const int x, const int y,
                            const char *left_text, esphome::Color left_color,
                            const char *right_text, esphome::Color right_color,
                            ui::Box &prev_box,
                            const int spacing = 0,
         ...
         * @param left_text Pointer to the start of a c-string representing the first (left-most) text blob.
         * @param left_color Render left_text with this color.
         * @param right_text Pointer to the start of a c-string representing the last (right-most) text blob.
         * @param right_color Render right_text with this color
         * @param prev_box Reference object to place aggregate dimensions into.
         * @param spacing number of pixels to place inbetween left and right objects.
         * @param align Determines how to interpret x, y
         */
        //
        it->printf(x, y, font, left_color, align, "%s", left_text);         // Draw left part

        int tx1, ty1, tw, th;
        it->get_text_bounds(x, y, left_text, font, align, &tx1, &ty1, &tw, &th);  // Compute where to start the right text
        const ui::Box lb{tx1, ty1, tw, th};
        const int left_width = tw;
        const int x2 = x + left_width + spacing;

        it->printf(x2, y, font, right_color, align, "%s", right_text);      // Draw right part
