...
namespace ui {
    ...
    struct MultiColorString {
        virtual void update_colors(esphome::Color &user, esphome::Color &message) = 0;
    };
    TwitchStringComponents split_twitch_string(const std::string &input) {
        ...
        if (pos != std::string::npos) {
            ...
            std::size_t msg_start = pos + 1;         // Everything after ':' (skipping a following
            ...
            result.message = input;                  // No ':' found; treat the whole thing as the message
        }
        return result;
    }

    template <std::size_t BufSize>
    class TwitchStringWidget : public DynStringWidget<BufSize>, public MultiColorString {
    private:
    ...
    public:
        ...
        void write() override {
            if (this->abc.user == "" && this->abc.message == "") return;
            ...
            if (this->use_max_width_as_width && this->right_align) {
                ...
                const int x_draw = this->anchor.x + (this->max_width - curr_buf_width);
                ui::printf_dual(this->it, this->font, x_draw, y,
                                (this->abc.user + ": ").c_str(), WHITE,
                                this->abc.message.c_str(), YELLOW,
                                this->prev_box);
            } else {
